...
The Friemon Core daemon can be configured to act as a trusted "border
router", implementing the friemon wire protocol and relay, making
...
The ZeroMQ feature in Friemon Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building friemond:
...
For instance:
    ...
    $ friemond -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/friemond.tx.raw \
...
These options can also be provided in friemon.conf.
...
From the perspective of friemond, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into friemond directly. Furthermore, no information is
...
using. Friemond appends an up-counting sequence number to each
