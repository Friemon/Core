Friemon Core version 0.18.1 is now available from:

  <https://download.friemon.org/friemon-0.18.1/>
...
`/Applications/Friemon-Qt` (on Mac) or `friemond`/`friemon-qt` (on
Linux).
...
Friemon Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Friemon Core on unsupported systems.
...
10.10. Additionally, Friemon Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:18577:18577` (this is an extra `:18577` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@friemon.org) the Friemon Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 FRMN),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in FRMN per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/friemon-conf.md)
  about the `friemon.conf` file describes how to use it to configure
  Friemon Core.

- A new document introduces Friemon Core's BIP174 [Partially-Signed
  Friemon Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Friemon-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Friemon Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Friemon Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Friemon Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart friemond with the `-deprecatedrpc=generate`
...
- A new `friemon-wallet` tool is now distributed alongside Friemon
...
This section describes planned changes to Friemon Core that may affect
other Friemon software and services.

- Since version 0.16.0, Friemon Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Friemon Core 0.20 (expected about
  a year after 0.18), Friemon Core will default to native segwit
  ...
  to bech32 addresses, and if the Friemon Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Friemon Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Friemon Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Friemon Core
...
- A sub-project of Friemon Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Friemon Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Friemon Core's own implementation, although entropy
  gathered by Friemon Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Friemon Core a
...
- On macOS, Friemon Core now opts out of application CPU throttling
...
- #18577 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
